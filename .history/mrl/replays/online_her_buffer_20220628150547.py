...
class OnlineHERBuffer(mrl.Module):
    ...
    def __init__(
        ...
        self.buffer = None
        # can be manually set to save this replay buffer irrespective of config
        ...
        if self.goal_shape:
            ...
            if hasattr(self, 'ag_curiosity') and self.ag_curiosity.current_goals is not None:
                ...
                reward = self.env.compute_reward(achieved, behavioral, {
                                                 's': state, 'a': action, 'ns': next_state}).reshape(-1, 1)
            else:
                behavioral = desired
        ...
        if self.goal_shape:
            ...
            if has_config_her:
                ...
                if hasattr(self, 'goal_reward'):
                    rewards = self.goal_reward(ags, goals, {
                                               's': states, 'a': actions, 'ns': next_states}).reshape(-1, 1).astype(np.float32)
                else:
                    rewards = self.env.compute_reward(ags, goals, {
                                                      's': states, 'a': actions, 'ns': next_states}).reshape(-1, 1).astype(np.float32)
