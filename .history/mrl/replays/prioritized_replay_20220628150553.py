...
class EntropyPrioritizedOnlineHERBuffer(mrl.Module):
    ...
    def __init__(
        ...
        super().__init__(module_name, required_agent_modules=[
            'env', 'replay_buffer'], locals=locals())
    ...
    def _setup(self):
        ...
        for name, shape in items:
            self.buffer['buffer_' +
                        name] = RingBuffer(self.ag_buffer.maxlen, shape=shape)
    ...
    def fit_density_model(self):
        ...
        # [num_episodes, episode_len * goal_dim]
        X_train = ag.reshape(-1, self.traj_len * ag.shape[-1])

        self.clf = mixture.BayesianGaussianMixture(
            weight_concentration_prior_type="dirichlet_distribution", n_components=3)
        ...
        self.buffer.buffer_entropy.data[:
                                        self.size] = pred.reshape(-1, 1).copy()
        ...
        for i in range(self.n_envs):
            if exp.trajectory_over[i]:
                ...
                if not isinstance(self.clf, int):
                    # [episode_len, goal_dim]
                    ag = [np.stack(a) for a in zip(*self._subbuffers[i])][0]
    ...
    def update_priority(self):
        ...
        entropy_rank = rankdata(entropy_transition_total,
    ...
    def __call__(self, batch_size):
        ...
        entropy_trajectory = entropy_trajectory.reshape(-1, self.traj_len)[
            :, 0]
        ...
        batch_tidx = np.random.choice(
            num_trajectories, size=batch_size, p=p_trajectory)
        batch_idxs = self.traj_len * batch_tidx + \
            np.random.choice(self.traj_len, size=batch_size)
    ...
    def load(self, save_folder):
        load_path = os.path.join(
            save_folder, "{}.pickle".format(self.module_name))
        if os.path.exists(load_path):
            ...
            self.logger.log_color(
                '###############################################################', '', color='red')
            self.logger.log_color(
                'WARNING', 'Replay buffer is not being loaded / was not saved.', color='cyan')
            self.logger.log_color(
                'WARNING', 'Replay buffer is not being loaded / was not saved.', color='red')
            self.logger.log_color(
                'WARNING', 'Replay buffer is not being loaded / was not saved.', color='yellow')
            self.logger.log_color(
                '###############################################################', '', color='red')
