...
class RawKernelDensity(mrl.Module):
    ...
    def __init__(self, item, optimize_every=10, samples=10000, kernel='gaussian', bandwidth=0.1, normalize=True,
        ...
        super().__init__('{}_kde{}'.format(item, tag),
                         required_agent_modules=[buffer_name], locals=locals())
    ...
    def _optimize(self, force=False):
        ...
        if force or (self.step % self.optimize_every == 0 and len(buffer)):
            ...
            if self.normalize:
                self.kde_sample_mean = np.mean(
                    kde_samples, axis=0, keepdims=True)
                self.kde_sample_std = np.std(
                    kde_samples, axis=0, keepdims=True) + 1e-4
                kde_samples = (kde_samples - self.kde_sample_mean) / \
                    self.kde_sample_std
            ...
            if self.log_entropy and hasattr(self, 'logger') and self.step % 250 == 0:
                ...
                entropy = - \
                    self.fitted_kde.score(s)/num_samples + \
                    np.log(self.kde_sample_std).sum()
                self.logger.add_scalar(
                    'Explore/{}_entropy'.format(self.module_name), entropy, log_every=500)
    ...
    def evaluate_elementwise_entropy(self, samples, beta=0.):
        ...
        log_px = self.fitted_kde.score_samples(
    ...
    def save(self, save_folder):
        self._save_props(['kde', 'kde_sample_mean',
                         'kde_sample_std', 'fitted_kde', 'ready'], save_folder)

    def load(self, save_folder):
        self._load_props(['kde', 'kde_sample_mean',
                         'kde_sample_std', 'fitted_kde', 'ready'], save_folder)
...
class RawJointKernelDensity(mrl.Module):
    ...
    def __init__(self, items, optimize_every=10, samples=10000, kernel='gaussian', bandwidth=0.1, normalize=True, log_entropy=False, tag=''):
        super().__init__('{}_kde{}'.format("".join(items), tag),
                         required_agent_modules=['replay_buffer'], locals=locals())
    ...
    def _optimize(self, force=False):
        ...
        if force or (self.step % self.optimize_every == 0 and len(buffers[0])):
            ...
            if self.log_entropy and hasattr(self, 'logger') and self.step % 250 == 0:
                ...
                entropy = - \
                    self.fitted_kde.score(s)/num_samples + \
                    np.log(self.kde_sample_std).prod()
                self.logger.add_scalar(
                    'Explore/{}_entropy'.format(self.module_name), entropy, log_every=500)
...
class RandomNetworkDensity(mrl.Module):
    ...
    def __init__(self, item, optimize_every=1, batch_size=256, layers=(256, 256)):
        ...
        super().__init__('{}_rnd'.format(item), required_agent_modules=[
            'replay_buffer'], locals=locals())
    ...
    def _init_from_sample(self, x):
        ...
        self.tgt_net = MLP(
            input_size, output_size=self.layers[-1], hidden_sizes=self.layers[:-1])
        self.prd_net = MLP(
            input_size, output_size=self.layers[-1], hidden_sizes=self.layers[:-1])
        ...
        self.optimizer = torch.optim.SGD(
            self.prd_net.parameters(), lr=0.1, weight_decay=1e-5)
...
class FlowDensity(mrl.Module):
    ...
    def __init__(self, item, optimize_every=2, batch_size=1000, lr=1e-3, num_layer_pairs=3, normalize=True):
        ...
        super().__init__('{}_flow'.format(item),
                         required_agent_modules=['replay_buffer'], locals=locals())
    ...
    def _init_from_sample(self, x):
        ...
        self.flow_model = RealNVP(input_channel=self.input_channel,
                                  lr=self.lr, num_layer_pairs=self.num_layer_pairs, dev=self.dev)
