...
class GoalEnvReward(mrl.Module):
    ...
    def _setup(self):
        ...
        assert hasattr(
            self.env, 'compute_reward'), "Environment must have compute reward defined!"

    def __call__(self, achieved_goals, goals, info):
        return self.env.compute_reward(achieved_goals, goals, info)


class GoalEnvAchieved(mrl.Module):
    def __init__(self):
    ...
    def _setup(self):
        assert self.env.goal_env, "Environment must be a goal environment!"
        assert hasattr(
            self.env, 'achieved_goal'), "Environment must have achieved_goal defined!"
...
class NeighborReward(mrl.Module):
    def __init__(self, max_neighbor_distance=1, optimize_every=5, batch_size=1000, temperature=1.):
        ...
        # this is the number of steps from which to count two goals as neighbors.
    ...
    def _optimize(self):
        ...
        if self.step % self.optimize_every == 0 and len(ag_buffer):
            sample_idxs = np.random.randint(
                len(ag_buffer), size=self.batch_size)
            ...
            # get embeddings
            embs = self.neighbor_embedding_network(
                torch.cat((ags, pos, neg), dim=0))
            ...
            loss = F.binary_cross_entropy_with_logits(torch.exp(pos_logits), torch.ones_like(pos_logits) * 0.99) +\
                F.binary_cross_entropy_with_logits(
                    torch.exp(neg_logits), torch.ones_like(pos_logits) * 0.01)
