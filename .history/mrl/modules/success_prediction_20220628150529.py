...
class GoalSuccessPredictor(mrl.Module):
    ...
    def _setup(self):
        ...
        self.optimizer = torch.optim.Adam(
            self.goal_discriminator.model.parameters())

    def _optimize(self):
        self.opt_steps += 1

        if len(self.replay_buffer.buffer.trajectories) > self.batch_size and self.opt_steps % self.optimize_every == 0:
            trajs = self.replay_buffer.buffer.sample_trajectories(
                self.batch_size, group_by_buffer=True, from_m_most_recent=self.history_length)
            successes = np.array(
                [np.any(np.isclose(traj, 0.), axis=0) for traj in trajs[2]])
            ...
            if hasattr(self, 'logger'):
                self.logger.add_histogram(
                    'predictions', torch.sigmoid(outputs), self.log_every)
