...
class EnvModule(mrl.Module):
    ...
    def __init__(
        ...
        if isinstance(env, str):
            ...
            env_list = [make_env_by_id(env, seed, i, episode_life)
                        for i in range(num_envs)]
        else:
            sample_env = make_env(env, seed, 0)()
            assert isinstance(
                sample_env, gym.core.Env), "Only gym environments supported for now!"
        ...
        if isinstance(self.action_space, gym.spaces.Discrete):
            ...
            assert isinstance(
                self.action_space, gym.spaces.Box), "Only Box/Discrete actions supported for now!"
        ...
        if isinstance(self.observation_space, gym.spaces.Dict):
            if goal_modalities[0] in self.observation_space.spaces:
                ...
                for key in goal_modalities:
                    ...
                    self.goal_dim += int(
                        np.prod(self.env.observation_space[key].shape))
            state_dim = 0
    ...
    def _init():
        ...
        if is_atari:
            env = wrap_deepmind(env, episode_life=episode_life,
                                clip_rewards=False, frame_stack=False, scale=False)
