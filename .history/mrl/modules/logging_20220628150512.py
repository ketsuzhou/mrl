...
class Logger(mrl.Module):
    ...
    def __init__(self, average_every=100):
        super().__init__('logger', required_agent_modules=[
            'env'], locals=locals())
    ...
    def update_csv(self, tag, value, step):
        ...
        path = os.path.join(self.agent_folder, self.agent_name +
                            '__' + tag.replace('/', '__') + '.csv')
    ...
    def add_embedding(self, tag, values, log_every=1000, step=None, **kwargs):
        ...
        if step - self.last_log_step[tag] >= log_every:
            ...
            self.writer.add_embedding(
                mat=values, tag=tag, global_step=step, **kwargs)
    ...
    def save_config(self):
        config_json = convert_json(
            {**self.config, **record_attrs(self.module_dict.values())})
    ...
    def flush_console(self):
        table = [('Environment steps', self.steps), ('Total episodes', self.episodes),
                 ('Avg rewards (last {})'.format(self.average_every),
                  np.mean(self.episode_rewards[-self.average_every:])),
                 ('Avg episode len (last {})'.format(self.average_every),
                  np.mean(self.episode_steps[-self.average_every:]))
                 ]
        for k, v in self.tabular.items():
            table.append(('Avg ' + k + ' (last {})'.format(self.average_every),
                         np.mean(v[-self.average_every:])))
        table = tabulate(
            table, headers=['Tag', 'Value'], tablefmt="psql", floatfmt="8.1f")
    ...
    def _process_experience(self, experience):
        ...
        if self.steps % self.log_every_n_steps < self.env.num_envs:
            self.flush_console()
            self.add_scalar('Train/Episode_rewards',
                            np.mean(self.episode_rewards[-30:]))
            self.add_scalar('Train/Episode_steps',
                            np.mean(self.episode_steps[-30:]))
...
def record_attrs(module_list):
    ...
    for module in module_list:
        res['module_' + module.module_name] = convert_json(
            strip_config_spec(module.config_spec), dict_to_str=True)
    return res
