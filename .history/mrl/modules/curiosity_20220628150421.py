...
class AchievedGoalCuriosity(mrl.Module):
    ...
    def __init__(self, num_sampled_ags=500, max_steps=50, keep_dg_percent=-1e-1, randomize=False, use_qcutoff=True):
        super().__init__('ag_curiosity',
                         required_agent_modules=[
                             'env', 'replay_buffer', 'actor', 'critic'],
                         locals=locals())
    ...
    def _manage_resets_and_success_behaviors(self, experience, close):
        ...
        for i, over in enumerate(experience.trajectory_over):
            ...
            # if not over and success, modify go_explore; maybe overshoot goal?
            if not over and close[i]:
                self.is_success[i] += 1.
                self.go_explore[i] += 1.

                if not self.config.get('never_done') and np.random.random() < self.config.overshoot_goal_percent:
                    step_amount = experience.next_state['achieved_goal'][i] - \
                        experience.state['achieved_goal'][i]
    ...
    def _overshoot_goals(self, experience, overshooting_idxs, overshooting_proposals):
        ...
        states = np.tile(
            experience.reset_state['observation'][overshooting_idxs, None, :], (1, num_proposals, 1))
        states = np.concatenate(
            (states, overshooting_proposals), -1).reshape(num_proposals * num_idxs, -1)
        ...
        goal_values = self.score_goals(
            overshooting_proposals, AttrDict(q_values=q_values, states=states))

        if self.config.dg_score_multiplier > 1. and self.dg_kde.ready:
            dg_scores = self.dg_kde.evaluate_log_density(
                overshooting_proposals.reshape(num_proposals * num_idxs, -1))
        ...
        # shape(sampled_ags) = n_envs x num_proposals
        chosen_idx = np.eye(num_proposals)[chosen_idx]
        # n_envs x goal_feats
        chosen_ags = np.sum(overshooting_proposals *
                            chosen_idx[:, :, None], axis=1)
    ...
    def _process_experience(self, experience):
        ...
        if overshooting_idxs and len(ag_buffer):
            self._overshoot_goals(
                experience, overshooting_idxs, overshooting_proposals)
        ...
        if np.any(experience.trajectory_over) and len(ag_buffer):
            ...
            sample_idxs = np.random.randint(
                len(ag_buffer), size=self.num_sampled_ags * self.n_envs)
            sampled_ags = ag_buffer.get_batch(sample_idxs)
            sampled_ags = sampled_ags.reshape(
                self.n_envs, self.num_sampled_ags, -1)

            # compute the q-values of both the sampled achieved goals and the current goals
            states = np.tile(
                experience.reset_state['observation'][:, None, :], (1, self.num_sampled_ags, 1))
            states = np.concatenate(
                (states, sampled_ags), -1).reshape(self.num_sampled_ags * self.n_envs, -1)
            states_curr = np.concatenate(
                (experience.reset_state['observation'], self.current_goals), -1)
            ...
            if self.use_qcutoff:
                ...
                if len(self.successes_deque) == 10:
                    self.min_cutoff = max(self.min_min_cutoff, min(
                        np.min(q_values), self.min_cutoff))
                    ...
                    if intrinsic_success_percent >= self.config.cutoff_success_threshold[1]:
                        ...
                        self.cutoff = max(
                            min(self.config.initial_cutoff, self.cutoff + 1.), self.min_min_cutoff)
                        self.successes_deque.clear()
                ...
                min_q_values = np.min(
                    q_values, axis=1, keepdims=True)  # num_envs x1
                q_values[bad_q_idxs] *= -1
            ...
            if self.config.dg_score_multiplier > 1. and self.dg_kde.ready:
                ...
                dg_scores = dg_scores.reshape(
                    self.n_envs, self.num_sampled_ags)
                goal_values[dg_scores > -
                            np.inf] *= self.config.dg_score_multiplier
            ...
            if self.randomize:  # sample proportional to the absolute score
                ...
                normalized_values = abs_goal_values / \
                    np.sum(abs_goal_values, axis=1, keepdims=True)
                chosen_idx = (normalized_values.cumsum(1) > np.random.rand(
                    normalized_values.shape[0])[:, None]).argmax(1)
            else:  # take minimum
                chosen_idx = np.argmin(goal_values, axis=1)

            # shape(sampled_ags) = n_envs x num_sampled_ags
            chosen_idx = np.eye(self.num_sampled_ags)[chosen_idx]
            if q_values is not None:
                chosen_q_val = (
                    chosen_idx * q_values).sum(axis=1, keepdims=True)
            # n_envs x goal_feats
            chosen_ags = np.sum(sampled_ags * chosen_idx[:, :, None], axis=1)
            ...
            if hasattr(self, 'curiosity_alpha'):
                if self.use_qcutoff:
                    ...
                    replace_goal = (np.random.random(
                        (self.n_envs, 1)) > self.curiosity_alpha.alpha).astype(np.float32)
            ...
            if hasattr(self, 'logger') and len(self.successes) > 50:
                if q_values is not None:
                    self.logger.add_histogram(
                        'Explore/Goal_q', replace_goal * chosen_q_val + (1 - replace_goal) * curr_q)
                self.logger.add_scalar(
                    'Explore/Intrinsic_success_percent', np.mean(self.successes))
    ...
    def compute_q(self, numpy_states):
        numpy_states = self.state_normalizer(
            numpy_states, update=False).astype(np.float32)
    ...
    def save(self, save_folder):
        # can restart keeping track of successes / go explore
        self._save_props(['cutoff', 'min_cutoff'], save_folder)
...
class SuccessAchievedGoalCuriosity(AchievedGoalCuriosity):
    ...
    def score_goals(self, sampled_ags, info):
        ...
        scores = self.success_predictor(info.states).reshape(
            num_envs, num_sampled_ags)  # these are predicted success %
        # rank by distance to 0.5, lower is closer to 0.5
        scores = -0.5 + np.abs(scores - 0.5)
...
class DensityAchievedGoalCuriosity(AchievedGoalCuriosity):
    ...
    def score_goals(self, sampled_ags, info):
        ...
        flattened_sampled_ags = sampled_ags.reshape(
            num_envs * num_sampled_ags, -1)
        ...
        # make negative / reverse order so that lower is better.
...
class EntropyGainScoringGoalCuriosity(AchievedGoalCuriosity):
    ...
    def score_goals(self, sampled_ags, info):
        ...
        indexer = np.arange(num_envs * num_sampled_ags).reshape(-1,
                                                                1) + np.arange(num_ags).reshape(1, -1)
        ...
        joint_candidate_bgags = np.concatenate(
            [candidate_bgs_repeat, ags_samples], axis=-1)
        joint_candidate_bgags = joint_candidate_bgags.reshape(
            num_envs * num_sampled_ags * num_ags, -1)

        # score the sampled_ags to get log densities, and exponentiate to get densities
        joint_candidate_score = self.bgag_kde.evaluate_log_density(
            joint_candidate_bgags)
        ...
        candidate_bgs_score = candidate_bgs_score.reshape(
            num_envs * num_sampled_ags, num_ags)  # these are log densities
        ...
        sampled_ag_entr_new = self.ag_kde.evaluate_elementwise_entropy(
            candidate_bgs, beta=beta)
        sampled_ag_entr_old = self.ag_kde.evaluate_elementwise_entropy(
            candidate_bgs, beta=0.)
        sampled_ag_entr_gain = sampled_ag_entr_new - sampled_ag_entr_old
        # Normalize by beta # TODO: Get rid of this part if not necessary
        sampled_ag_entr_gain /= beta
...
class CuriosityAlphaMixtureModule(mrl.Module):
    ...
    def __init__(self, optimize_every=100):
        super().__init__('curiosity_alpha',
                         required_agent_modules=[
                             'ag_curiosity', 'ag_kde', 'replay_buffer'],
                         locals=locals())
        self.samples = None
        self.bandwidth = None
        self.kernel = None
    ...
    def _optimize(self):
        ...
        if self.step % self.optimize_every == 0 and len(buffer):
            ...
            kde_samples = (kde_samples - self.ag_kde.kde_sample_mean) / \
                self.ag_kde.kde_sample_std
            ...
            self._alpha = 1. / \
                max((self._beta + np.mean(log_p_dg) - np.mean(log_p_ag)), 1.)

            # Occasionally log the alpha
            self.logger.add_scalar(
                'Explore/curiosity_alpha', self._alpha, log_every=500)
            self.logger.add_tabular('Curiosity_alpha', self._alpha)

    def save(self, save_folder):
        self._save_props(
            ['kde', 'samples', 'bandwidth', 'kernel'], save_folder)
