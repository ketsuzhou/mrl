...
def make_ddpg_agent(base_config=default_ddpg_config,
    ...
    config.agent_name = make_agent_name(
        config, agent_name_attrs, prefix=args.prefix)
    ...
    base_modules = {
        ...
        for k, v in dict(module_train=StandardTrain(),
                         ...
                         module_state_normalizer=Normalizer(
                             MeanStdNormalizer()),
    ...
    if hasattr(args, 'eval_env') and args.eval_env is not None:
        if type(args.eval_env) is str:
            def eval_env(): return gym.make(args.eval_env)
        else:
    ...
    config.module_eval_env = EnvModule(
        eval_env, num_envs=config.num_eval_envs, name='eval_env', seed=config.seed + 1138)

    layer_norm = nn.LayerNorm if (
        hasattr(args, 'layer_norm') and args.layer_norm) else nn.Identity
    ...
    if e.goal_env:
        # important for standard Gym goal environments, which are never done
