    ...
    def __call__(self, state, greedy=False):
        ...
        if self.training:
            if self.config.get('initial_explore') and len(self.replay_buffer) < self.config.initial_explore:
                res = np.array([self.env.action_space.sample()
                               for _ in range(self.env.num_envs)])
            elif hasattr(self, 'ag_curiosity'):
                state = self.ag_curiosity.relabel_state(state)

        # flatten goal environments
        state = flatten_state(
            state, self.config.modalities + self.config.goal_modalities)
        ...
        if self.training and not greedy:
            ...
            if self.config.get('eexplore'):
                ...
                randoms = np.random.random(
                    action.shape) * (2 * action_scale) - action_scale
...
class BatchConstrainedPolicy(mrl.Module):
    ...
    def __call__(self, state, greedy=False):
        ...
        # flatten goal environments, # batch x state_dim
        state = flatten_state(
            state, self.config.modalities + self.config.goal_modalities)
        if hasattr(self, 'state_normalizer'):
            state = self.state_normalizer(state, update=self.training)
        ...
        # batch x num_proposals x action_dim
        action_proposals = self.actor(state)
        states = torch.repeat_interleave(state, action_proposals.shape[1], 0)
        q_values = self.critic(
            states, action_proposals.reshape(-1, action_proposals.shape[-1]))
        q_values = q_values.reshape(
            state.shape[0], action_proposals.shape[1])  # batch x num_proposals
        best_actions = torch.argmax(
            q_values, dim=-1, keepdims=True)  # batch x 1
        action = action_proposals.gather(1, torch.tile(
            best_actions[:, :, None], (1, 1, action_proposals.shape[2]))).squeeze(1)
        action = self.numpy(action)
    ...
    def optimize_from_batch(self, states, actions, rewards, next_states, gammas):
        ...
        if self.config.grad_value_clipping > 0.:
            torch.nn.utils.clip_grad_value_(
                self.critic_params, self.config.grad_value_clipping)
        ...
        if self.config.get('policy_opt_noise'):
            noise = torch.randn_like(
                a) * (self.config.policy_opt_noise * self.action_scale)
        ...
        if self.config.action_l2_regularization:
            actor_loss += self.config.action_l2_regularization * \
                F.mse_loss(a / self.action_scale, torch.zeros_like(a))
...
class TD3(OffPolicyActorCritic):
    ...
    def optimize_from_batch(self, states, actions, rewards, next_states, gammas):
        ...
        with torch.no_grad():
            ...
            a_next_max = (
                a_next_max + noise).clamp(-self.action_scale, self.action_scale)
        ...
        if self.config.grad_norm_clipping > 0.:
            torch.nn.utils.clip_grad_norm_(
                self.critic_params, self.config.grad_norm_clipping)
        if self.config.grad_value_clipping > 0.:
            torch.nn.utils.clip_grad_value_(
                self.critic_params, self.config.grad_value_clipping)
        ...
        if config.opt_steps % config.td3_delay == 0:
            ...
            if self.config.get('policy_opt_noise'):
                noise = torch.randn_like(
                    a) * (config.policy_opt_noise * self.action_scale)
                a = (a + noise).clamp(-self.action_scale, self.action_scale)
            actor_loss = -torch.min(self.critic(states, a)
                                    [:, -1], self.critic2(states, a)[:, -1]).mean()
            if self.config.action_l2_regularization:
                actor_loss += self.config.action_l2_regularization * \
                    F.mse_loss(a / self.action_scale, torch.zeros_like(a))
            ...
            if self.config.grad_norm_clipping > 0.:
                torch.nn.utils.clip_grad_norm_(
                    self.actor_params, self.config.grad_norm_clipping)
            if self.config.grad_value_clipping > 0.:
                torch.nn.utils.clip_grad_value_(
                    self.actor_params, self.config.grad_value_clipping)
...
class SAC(OffPolicyActorCritic):
    ...
    def optimize_from_batch(self, states, actions, rewards, next_states, gammas):
        ...
        with torch.no_grad():
            ...
            target = rewards + gammas * \
                (torch.min(q1, q2) - config.entropy_coef * logp_next)
            target = torch.clamp(target, *self.config.clip_target_range)
...
class BCDDPG(OffPolicyActorCritic):
    ...
    def optimize_from_batch(self, states, actions, rewards, next_states, gammas):
        ...
        with torch.no_grad():
            # batch x num_proposals x action_dim
            action_proposals = self.actor(next_states)
            _next_states = torch.repeat_interleave(
                next_states, action_proposals.shape[1], 0)
            q_values = self.critic_target(
                _next_states, action_proposals.reshape(-1, action_proposals.shape[-1]))
            q_values = q_values.reshape(
                next_states.shape[0], action_proposals.shape[1])  # batch x num_proposals
...
class SimpleConservativeSAC(OffPolicyActorCritic):
    ...
    def optimize_from_batch(self, states, actions, rewards, next_states, gammas):
        ...
        # CQL Bit
        random_actions = (torch.rand((len(states), self.env.action_dim))
                          * 2 * self.env.max_action - self.env.max_action).to(states.device)
