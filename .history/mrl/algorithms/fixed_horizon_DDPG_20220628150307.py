...
class FHDDPG(OffPolicyActorCritic):
    ...
    def optimize_from_batch(self, states, actions, rewards, next_states, gammas):
        ...
        q_next = self.critic_target(
            next_states, a_next_max)  # batch x n_horizons
        ...
        if hasattr(self, 'logger') and self.config.opt_steps % 100 == 0:
            self.logger.add_histogram(
                'Optimize/First_horizon_target', target[:, 0])
            self.logger.add_histogram(
                'Optimize/Last_horizon_target', target[:, -1])
        ...
        # update actor using ensemble of last 5 horizons
        actor_loss = -self.critic(states, a)[:, -5:].mean()
        if self.config.action_l2_regularization:
            actor_loss += self.config.action_l2_regularization * \
                F.mse_loss(a, torch.zeros_like(a))
