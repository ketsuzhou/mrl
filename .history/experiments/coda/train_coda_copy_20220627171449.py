...
def make_disentangled_fetch_env(envstr):
    if 'push' in envstr.lower():
        def env(): return TimeLimit(DisentangledFetchPushEnv(), 50)
        eval_env = env
        dummy_env = gym.make('FetchPush-v1')
    elif 'pick' in envstr.lower():
        def env(): return TimeLimit(DisentangledFetchPickAndPlaceEnv(), 50)
        eval_env = env
        dummy_env = gym.make('FetchPickAndPlace-v1')
    elif 'slide' in envstr.lower():
        def env(): return TimeLimit(DisentangledFetchSlideEnv(), 50)
        def eval_env(): return TimeLimit(DisentangledFetchSlideEnv(), 50)
...
def main(args):
    ...
    if gym.envs.registry.env_specs.get(args.envstr) is not None:
        ...
        args.env, args.eval_env, dummy_env, state_dims = make_disentangled_fetch_env(
            args.envstr)
        config.slot_state_dims = [
            np.arange(10)] + [10 + 12 * i + np.arange(12) for i in range(1)]
        config.slot_action_dims = None
        dummy_env_config = None

        def reward_fn(s, a, ns, g): return dummy_env.compute_reward(
            ns[:, 10:13], g, None)[:, None]
        ...
        def reward_fn(s, a, ns, g): return dummy_env.compute_reward(
            ns[:, dummy_env.ag_dims], g, None)[:, None]
        ...
        if args.relabel_type == 'online_attn':
            model = SimpleStackedAttn(10 + 12 * n + 4,
                                      ...
                                      num_heads=5)
            config.mask_module = CodaAttentionBasedMask(
                model=model, optimize_every=2, batch_size=512)
    ...
    if args.relabel_type is not None:
        if args.relabel_type == 'ground_truth':
            ...
            agent.get_coda_mask = partial(
                agent.coda_attention_model.get_mask, THRESH=args.thresh)
        else:
            raise NotImplementedError()
    ...
    if args.visualize_trained_agent:
        ...
        agent.logger.log_color(
            f'Initial test reward ({num_eps} eps):', '{:.2f}'.format(res))
        ...
        for epoch in range(int(args.max_steps // args.epoch_len)):
            ...
            # VIZUALIZE GOALS
            if args.save_embeddings:
                idxs = np.random.choice(len(s1_buff), size=min(
                    len(s1_buff), 1000), replace=False)
                last_idxs = np.arange(
                    max(0, len(s1_buff) - args.epoch_len), len(s1_buff))

                rands1 = s1_buff.get_batch(idxs)
                rands1 = np.concatenate(
                    (rands1[:, 0, :3], rands1[:, 1, 10:13]), 1)
            ...
            res = np.mean(agent.eval(num_episodes=num_eps).rewards)
            agent.logger.log_color(
                f'Test reward ({num_eps} eps):', '{:.2f}'.format(res))
            agent.logger.log_color('Epoch time:', '{:.2f}'.format(
                time.time() - t), color='yellow')

            if args.relabel_type is not None:
                agent.logger.log_color('Coda buffer size:', len(
                    agent.replay_buffer.coda_buffer))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--parent_folder', default='/tmp/test_coda2',
                        type=str, help='where to save progress')
    parser.add_argument('--prefix', type=str, default='coda',
                        help='Prefix for agent name (subfolder where it is saved)')
    parser.add_argument('--envstr', default="disentangledpush",
                        type=str, help="env string [see code]")
    ...
    parser.add_argument('--tb', default='CODA', type=str,
    ...
    parser.add_argument('--train_dt', default=0.05,
                        type=float, help='training distance threshold')

    # Coda Args
    parser.add_argument("--relabel_type", default='push_heuristic',
                        type=str, help='type of relabeling to do')
    ...
    parser.add_argument("--max_coda_ratio", default=0.75,
                        type=float, help='Max proportion of coda:real data')
    parser.add_argument("--coda_samples_per_pair", default=2, type=int)
    parser.add_argument("--coda_every", default=250, type=int)
    parser.add_argument("--coda_source_pairs", default=2000, type=int)
    parser.add_argument("--min_experience_to_train_coda_attn",
                        default=25000, type=int)
    parser.add_argument("--coda_buffer_size", default=3000000, type=int,
                        help="Size of Coda Buffer (== replay_size if None)")
    parser.add_argument("--add_bad_dcs", action='store_true',
                        help="add entangled samples to the coda buffer?")
