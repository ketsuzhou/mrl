...
class GoalBasedSawyerLift(SawyerLift, gym.GoalEnv):
    ...
    def compute_reward(self, ag, dg, info):
        ...
        ns = info['ns']
        if len(ns.shape) == 1:
            # Penalize successes that move
            reward -= (0.2 *
                       np.tanh(np.mean(np.abs(info['s'][7:17] - ns[7:17]), axis=-1)) * success)
        else:
            # Penalize successes that move
            reward -= (0.2*np.tanh(
                np.mean(np.abs(info['s'][:, 7:17] - ns[:, 7:17]), axis=-1)) * success)
        ...
        if self.use_dense:
            if len(ns.shape) == 1:
                ...
                reward -= np.linalg.norm(ns[:, :3] -
                                         ns[:, 7:10], axis=-1) * failure
    ...
    def step(self, action):
        ...
        reward = self.compute_reward(
            ag, self.goal, {'s': obs, 'ns': self._prev_state})
        self._prev_state = obs
        obs = {
            'observation': obs,
            'achieved_goal': ag,
...
class SawyerReach(SawyerLift, gym.GoalEnv):
    ...
    def __init__(self):
        ...
        o = self._get_observation()
        self._obs_keys = ['eef_pos', 'joint_pos', 'joint_vel',
                          'gripper_qpos', 'gripper_qvel']  # , 'cube_pos', 'cube_quat']
        observation_space = gym.spaces.Box(-np.inf, np.inf,
                                           (sum([len(o[k]) for k in self._obs_keys]),))
        self.ag = lambda obs: obs[:3]
    ...
    def reset(self):
        ...
        proposal = init_pos + \
            np.random.uniform(-0.2, 0.2, 3) + np.array([0., 0., 0.15])
    ...
    def compute_reward(self, achieved_goal, desired_goal, info):
        ...
        if len(achieved_goal.shape) == 1:
            # Penalize successes that move
            reward -= (0.2*np.tanh(
                np.mean(np.abs(info['s'][0:10] - info['ns'][0:10]), axis=-1)) * success)
        else:
            # Penalize successes that move
            reward -= (0.2*np.tanh(
                np.mean(np.abs(info['s'][:, 0:10] - info['ns'][:, 0:10]), axis=-1)) * success)
    ...
    def step(self, action):
        ...
        reward = self.compute_reward(
            ag, self.goal, {'s': self._prev_state, 'ns': obs})
        self._prev_state = obs
        obs = {
            'observation': obs,
            'achieved_goal': ag,
