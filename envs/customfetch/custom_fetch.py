...
def sample_goal(initial_gripper_xpos, np_random, target_range, target_offset, height_offset, internal_goal,
    ...
    obj_goal = initial_gripper_xpos[:3] + \
        np_random.uniform(-target_range, target_range, size=3)
    obj_goal += target_offset
    obj_goal[2] = height_offset

    if internal_goal in [GoalType.GRIP, GoalType.OBJ_GRIP]:
        grip_goal = initial_gripper_xpos[:3] + \
            np_random.uniform(-0.15, 0.15, size=3) + np.array([0., 0., 0.15])
...
class StackEnv(fetch_env.FetchEnv, EzPickle):
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        if self.external_goal == GoalType.OBJ_GRIP:
            if len(achieved_goal.shape) == 1:
                ...
                achieved_internal_goals = np.split(
                    achieved_goal[:, :-3], self.n, axis=1)
    ...
    def _get_obs(self):
        ...
        for i in range(self.n):
            ...
            obj_feats.append([object_pos, object_rel_pos,
                             object_rot, object_velp, object_velr])
    ...
    def _sample_goal(self):
        bottom_box = self.initial_gripper_xpos[:3] + \
            self.np_random.uniform(-self.target_range,
                                   self.target_range, size=3)
        # self.sim.data.get_joint_qpos('object0:joint')[:3]
        bottom_box[2] = self.height_offset
...
class PickPlaceEnv(FetchPickAndPlaceEnv):
    ...
    def _sample_goal(self):
        ...
        if self.np_random.uniform() < self.in_air_percentage:
            height_offset += self.np_random.uniform(
                self.minimum_air, self.maximum_air)
...
class PushRandGoalDistEnv(PushEnv):
    ...
    def _get_obs(self):
        ...
        if self.done_init:
            ...
            observation['achieved_goal'] = np.dot(
                self.rand_goal_W, observation['achieved_goal'])
...
class PushGoalVisualizer(fetch_env.FetchEnv, EzPickle):
    ...
    def _render_callback(self):
        ...
        self.sim.model.site_pos[site_id] = self.goal - \
            sites_offset[0] + np.array([0., 0., 0.05])
...
class PPGoalVisualizer(fetch_env.FetchEnv, EzPickle):
    ...
    def _render_callback(self):
        ...
        site_id = self.sim.model.site_name2id('target1')
        self.sim.model.site_pos[site_id] = self.goal - \
            sites_offset[0] + np.array([-0.01, 0., 0.008])
...
class PushLeft(fetch_env.FetchEnv, EzPickle):
    ...
    def _env_setup(self, initial_qpos):
        ...
        self.initial_gripper_xpos = np.array(
            [1.34182673, 0.74891285, 0.41317183])
        if self.has_object:
    ...
    def _sample_goal(self):
        if self.has_object:
            if self.np_random.random() < 0.15:
                ...
                goal = self.initial_gripper_xpos[:3] + \
                    self.np_random.uniform(0., self.target_range, size=3)
            goal += self.target_offset
            goal[2] = self.height_offset
            if self.target_in_the_air and self.np_random.uniform() < 0.5:
                goal[2] += self.np_random.uniform(0, 0.45)
...
class DictPush(FetchPushEnv):
    def __init__(self):
        ...
        self.observation_space = spaces.Dict(dict(
            ...
            achieved_goal=spaces.Box(-np.inf, np.inf,
                                     shape=obs['achieved_goal'].shape, dtype='float32'),
            gripper=spaces.Box(-np.inf, np.inf,
                               shape=obs['gripper'].shape, dtype='float32'),
            object=spaces.Box(-np.inf, np.inf,
                              shape=obs['object'].shape, dtype='float32'),
            relative=spaces.Box(-np.inf, np.inf,
                                shape=obs['relative'].shape, dtype='float32'),
        ))
    ...
    def _get_obs(self):
        ...
        return {
            # hack to get around gym's GoalEnv checks
            'observation': np.zeros(0),
...
class DictPushAndReach(FetchPushEnv):
    def __init__(self):
        ...
        self.observation_space = spaces.Dict(dict(
            ...
            relative=spaces.Box(-np.inf, np.inf,
                                shape=obs['relative'].shape, dtype='float32'),
            gripper_goal=spaces.Box(-np.inf, np.inf,
                                    shape=(3,), dtype='float32'),
            object_goal=spaces.Box(-np.inf, np.inf,
                                   shape=(3,), dtype='float32')
        ))
...
class SlideNEnv(fetch_env.FetchEnv, EzPickle):
    def __init__(self,
                 ...
                 **kwargs):
        self.n = n
        self.disentangled_idxs = [
            np.arange(10)] + [10 + 12*i + np.arange(12) for i in range(n)]
        self.disentangled_goal_idxs = [3*i + np.arange(3) for i in range(n)]
        self.ag_dims = np.concatenate(
            [a[:3] for a in self.disentangled_idxs[1:]])
...
class FetchHookSweepAllEnv(fetch_env.FetchEnv):
    ...
    def render(self, mode="human", *args, **kwargs):
        ...
        if mode == 'rgb_array':
            ...
            data = self._get_viewer(mode='human').read_pixels(
    ...
    def _sample_goal(self):
        ...
        adj = self.np_random.random(
            size=(3, )) * np.array([0.2, 0.2, 0.]) - np.array([0.1, 0.1, 0])
        ...
        if self.place_two:
            ...
            goal_pos[:3] += self.np_random.random(size=(3, )) * np.array(
                [0.8, 0.8, 0.]) - np.array([0.4, 0.4, 0])
            goal_pos[3:] += self.np_random.random(size=(3, )) * np.array(
                [0.8, 0.8, 0.]) - np.array([0.4, 0.4, 0])
        else:
            if self.np_random.random() < 0.5:
    ...
    def _get_obs(self, force_original=False):
        ...
        for i in range(2):
            ...
            # rotations
            object_rot = rotations.mat2euler(
                self.sim.data.get_site_xmat(obj_labl))
            # velocities
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        if len(achieved_goal.shape) == 1:
            ...
            success = 1.
            on_table_bonus = np.clip(
                0.5 - np.abs(info['s'][2] - 0.42)*5., 0., 0.5)
        else:
