...
class EpsilonWrapper(object):
    ...
    def __init__(self, env, attrs=('distance_threshold', 'rotation_threshold'), compute_reward_with_internal=None):
        ...
        self.observation_space = spaces.Dict(dict(
            desired_goal=spaces.Box(-np.inf, np.inf, shape=(
                self.internal_len + len(self.attrs),), dtype='float32'),
            achieved_goal=spaces.Box(-np.inf, np.inf, shape=(
                self.internal_len + len(self.attrs),), dtype='float32'),
            observation=spaces.Box(-np.inf, np.inf,
                                   shape=(obs['observation'].shape[0],), dtype='float32'),
        ))
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        if self.compute_reward_with_internal:
            ...
            internal_reward = self.env.compute_reward(
                internal_achieved, internal_goal, info)
    ...
    def step(self, action):
        ...
        obs['achieved_goal'] = np.concatenate(
            [obs['achieved_goal'], self.defaults])
        obs['desired_goal'] = np.concatenate(
            [obs['desired_goal'], self.defaults])
...
class OldEpsilonWrapper(object):
    ...
    def __init__(self, env, epsilon, compute_reward_with_internal=None):
        ...
        self.observation_space = spaces.Dict(dict(
            desired_goal=spaces.Box(-np.inf, np.inf,
                                    shape=(obs['achieved_goal'].shape[0]*2,), dtype='float32'),
            achieved_goal=spaces.Box(-np.inf, np.inf, shape=(
                obs['achieved_goal'].shape[0]*2,), dtype='float32'),
            observation=spaces.Box(-np.inf, np.inf,
                                   shape=(obs['observation'].shape[0],), dtype='float32'),
        ))
    ...
    def step(self, action):
        obs, reward, done, info = self.env.step(action)
        obs['achieved_goal'] = np.concatenate(
            [obs['achieved_goal'], self.default_epsilon])
        obs['desired_goal'] = np.concatenate(
            [obs['desired_goal'], self.default_epsilon])
        reward = self.compute_reward(
            obs['achieved_goal'], obs['desired_goal'], info)
        return obs, reward, done, info
